...
#
# Usage:
#   sh tools/run/run-i915-passthrough.sh --preflight     # what is missing, and how
#   sh tools/run/run-i915-passthrough.sh                 # run, once bound
#
...
if [ "$drv" != "vfio-pci" ]; then
	...
	echo "Run: sh tools/run/run-i915-passthrough.sh --preflight" >&2
	exit 1
fi
...
#
# An ISO that carries root.img as a boot module makes the loader read the whole
