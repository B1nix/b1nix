...
#
# Usage: sh tools/run/debug/boot-timeline.sh [log ...]
#        sh tools/run/debug/boot-timeline.sh smoke_run/kde9.log
set -u
...
if [ $# -eq 0 ]; then
	echo "usage: sh tools/run/debug/boot-timeline.sh <serial log> [...]" >&2
	exit 2
fi
for f in "$@"; do
	timeline "$f"
