...
if [ "$B1NIX_TLS" != "none" ]; then
  echo "tools/ports/build-wget.sh: TLS provider '$B1NIX_TLS' requested, but wget TLS wiring is not enabled yet; building HTTP-only wget." >&2
fi
...
if [ ! -f "$PCRE2_PREFIX/lib/libpcre2-8.a" ]; then
  if ! "$ROOT_DIR/tools/ports/build-pcre2.sh" >/dev/null; then
    echo "tools/ports/build-wget.sh: PCRE2 build failed" >&2
    exit 1
  fi
fi
...
if [ ! -f "$OPENSSL_PREFIX/lib/libssl.a" ]; then
  if ! "$ROOT_DIR/tools/ports/build-openssl.sh" >/dev/null; then
    echo "tools/ports/build-wget.sh: OpenSSL build failed" >&2
    exit 1
  fi
fi
...
if [ ! -f "$LIBIDN2_PREFIX/lib/libidn2.a" ]; then
  if ! "$ROOT_DIR/tools/ports/build-libidn2.sh" >/dev/null; then
    echo "tools/ports/build-wget.sh: libidn2 build failed" >&2
    exit 1
  fi
fi
...
if [ ! -f "$LIBPSL_PREFIX/lib/libpsl.a" ]; then
  if ! "$ROOT_DIR/tools/ports/build-libpsl.sh" >/dev/null; then
    echo "tools/ports/build-wget.sh: libpsl build failed" >&2
    exit 1
  fi
fi
...
if [ ! -f "$SOURCE_READY" ]; then
  ...
  if [ -f "$tarball" ] && ! gzip -t "$tarball" 2>/dev/null; then
    echo "tools/ports/build-wget.sh: removing truncated cached archive $tarball" >&2
    rm -f "$tarball"
  fi
  if [ ! -f "$tarball" ]; then
    ...
    if command -v curl >/dev/null 2>&1; then
      ...
      echo "tools/ports/build-wget.sh: need host curl or wget to fetch $WGET_URL" >&2
      exit 1
    fi
    if ! gzip -t "$download"; then
      echo "tools/ports/build-wget.sh: downloaded archive is invalid: $WGET_URL" >&2
      rm -f "$download"
      exit 1
    fi
    mv "$download" "$tarball"
...
if [ -z "$ZLIB_PREFIX" ] || [ ! -f "$ZLIB_PREFIX/lib/libz.a" ]; then
  echo "tools/ports/build-wget.sh: zlib build failed" >&2
  exit 1
fi
