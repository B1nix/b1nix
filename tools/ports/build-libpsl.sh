...
if [ -z "$PYTHON_BIN" ]; then
  echo "tools/ports/build-libpsl.sh: need host python3 (or set PYTHON=/path/to/python)" >&2
  exit 1
fi
...
if [ ! -d "$SRC_DIR" ]; then
  ...
  if [ ! -f "$tmp" ]; then
    if command -v curl >/dev/null 2>&1; then
      ...
      echo "tools/ports/build-libpsl.sh: need host curl or wget to fetch $URL" >&2
      exit 1
    fi
  fi
