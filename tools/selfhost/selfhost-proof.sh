...
#
# Ships the self-host module (tools/selfhost/build-selfhost-module.sh) as the ram0
# btrfs Multiboot2 module and boots with b1nix.selfhostbuild, which makes the kernel
...
if [ "${SKIP_PACK:-0}" != "1" ] || [ ! -f "$IMG" ]; then
	...
	sh "$ROOT_DIR/tools/selfhost/build-selfhost-module.sh"
fi
...
"$ROOT_DIR/tools/images/mkiso.sh" "$@" >/dev/null
