...
#
#   sh tools/packages/build-deb.sh              # every source package
#   sh tools/packages/build-deb.sh b1nix-meta   # one of them
#
...
#
# The build runs inside the Debian chroot from tools/packages/debian-chroot.sh:
...
if git -C "$ROOT_DIR" describe --tags --exact-match --match 'v*' >/dev/null 2>&1; then
	...
	DEB_VERSION="$KVER+git$_date.$_sha-1"
fi
DATE=$(date -uR)
