...
# behind it.
#
#   sh tools/packages/publish-repo.sh                  # build the tree
#   SIGN_KEY=<key id> sh tools/packages/publish-repo.sh   # and sign Release
#
...
ls "$IN"/*.deb >/dev/null 2>&1 || die "no .deb files in $IN -- run tools/packages/build-deb.sh first"
