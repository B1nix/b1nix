...
#   sh tools/deb/debian-chroot.sh run <cmd...>  # run a command inside it
#   sh tools/deb/debian-chroot.sh shell         # interactive shell
#   sh tools/deb/debian-chroot.sh clean         # delete it
#
...
# Hub registry with curl -- the same flow tools/image/mk-debian-image.sh uses
...
# Nothing in here may hang for ever. apt waiting on a mirror, dpkg waiting on a
# lock it will never get, a build that loops: each of those used to sit until
# someone noticed. A step that takes longer than this is a bug to look at, not
# a slow machine to wait for.
