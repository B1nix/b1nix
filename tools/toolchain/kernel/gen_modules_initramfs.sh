...
# xxd is a vim binary and a host without vim has none; tools/toolchain/xxd-i.sh produces
# byte-identical output, which is the choice the Makefile already makes for its
# own .inc rules. This script was the one place that still demanded the real
# thing, and on a host without it the generated file held nothing but its
# header comment -- so B1NIX_MODULE_INITRAMFS_FILES was undefined and the
# kernel did not compile.
XXD_I="${XXD_I:-$(command -v xxd 2>/dev/null || echo "sh $(dirname "$0")/../xxd-i.sh")}"
...
mkdir -p "$(dirname "$OUT")"
: > "$OUT"
printf '/* Generated by tools/toolchain/kernel/gen_modules_initramfs.sh — do not edit. */\n' >> "$OUT"
for ko in "$@"; do
    name="$(basename "$ko" .ko)"
    $XXD_I -i -n "vfs_module_${name}_ko" "$ko" >> "$OUT"
done
