#!/usr/bin/env bash
# tools/toolchain/build-make.sh - Cross-build GNU Make for the b1nix target selected by
...
# `make -f Makefile` in-guest instead of a generated flat command list.
#
...
#
# Requires: cross-toolchain (tools/toolchain/build-toolchain.sh) + userspace libc
