...
# --- Patch 7: //build/config/rust.gni — real b1nix rust_abi_target ------------
# enable_rust + v8_enable_temporal_support are now ON (Temporal is Rust), so
# b1nix must map to its OWN Rust target (x86_64-unknown-b1nix — the cross-rust
# std sysroot built in build/rust-native/.../x86_64-unknown-linux-gnu/stage2),
# NOT the linux triple. Give b1nix a dedicated branch before the is_linux map.
# (This also clears the `assert(rust_abi_target != "")` the coverage config trips
# for every target.)
...
# rust.gni asserts the rust_abi_target appears in known-target-triples.txt.
...
# otherwise tries to copy it into the assembled sysroot and fails. Expect it only
# for the prebuilt chromium toolchain.
