...
#
#   sh tools/image/mk-debian-image.sh              # build/x86_64/debian.ext4
#   BUILD_DIR=smoke_run/debian-build sh tools/image/mk-debian-image.sh
#
...
# One copy of the harness, in tools/image/debian-stage.sh, rather than a copy
# here and another in the test: the test injects the same file into its scratch
...
# ── M127: resource control, which is systemd's whole model ─────────────────
# Not "the files exist": a slice that enforces MemoryMax, a slice that really
# gets the share of the CPU its CPUWeight asks for, a runaway that dies inside
# its own unit while the machine carries on, and pressure that moves.
CG=/sys/fs/cgroup

__ctl=$(cat $CG/cgroup.controllers 2>/dev/null)
__mycg=$(sed -n 's/^0:://p' /proc/self/cgroup)
__mymem=$(cat "$CG$__mycg/memory.current" 2>/dev/null || echo "")
say "SYSTEMD-SMOKE:   controllers='$__ctl' mycg=$__mycg memory.current=${__mymem:-none}"
if echo "$__ctl" | grep -q '\bcpu\b' && echo "$__ctl" | grep -q '\bmemory\b' &&
   echo "$__ctl" | grep -q '\bio\b' && echo "$__ctl" | grep -q '\bpids\b' &&
   [ -r "$CG$__mycg/memory.events" ] && [ -n "$__mymem" ] &&
   [ "$__mymem" -gt 0 ] 2>/dev/null; then
	say "SYSTEMD-SMOKE: ok resource-controllers"
else
	say "SYSTEMD-SMOKE: FAIL resource-controllers controllers='$__ctl' cg='$__mycg' mem='$__mymem'"
	[ -r "$CG$__mycg/memory.events" ] || say "SYSTEMD-SMOKE:   no $CG$__mycg/memory.events"
fi

# Before the runaway: a unit that only sleeps, so its cgroup can be read at
# leisure. If the limit is not even stored, or the members are not in the
# cgroup, the run below would take the machine down and say nothing about why.
systemctl reset-failed b1nix-memprobe.service >/dev/null 2>&1
timeout 60 systemd-run --unit=b1nix-memprobe.service -p MemoryMax=48M \
	-p MemoryAccounting=yes /bin/sleep 30 >/dev/null 2>&1
sleep 2
__pcg=$CG/system.slice/b1nix-memprobe.service
__ppid=$(systemctl show -p MainPID --value b1nix-memprobe.service 2>&1)
say "SYSTEMD-SMOKE:   memprobe pid=$__ppid max=[$(cat $__pcg/memory.max 2>&1)] cur=[$(cat $__pcg/memory.current 2>&1)] procs=[$(tr '\n' ' ' <$__pcg/cgroup.procs 2>&1)]"
say "SYSTEMD-SMOKE:   memprobe pidcg=[$(cat /proc/$__ppid/cgroup 2>&1)] events=[$(tr '\n' ' ' <$__pcg/memory.events 2>&1)]"
systemctl stop b1nix-memprobe.service >/dev/null 2>&1
systemctl reset-failed b1nix-memprobe.service >/dev/null 2>&1

# MemoryMax on a transient unit. `tail /dev/zero` grows one line buffer without
# end, so the only thing that can stop it is the limit.
systemctl reset-failed b1nix-memcap.service >/dev/null 2>&1
timeout 60 systemd-run --unit=b1nix-memcap.service -p MemoryMax=48M \
	-p MemoryAccounting=yes -p Restart=no \
	/bin/sh -c 'exec tail /dev/zero' >/dev/null 2>&1
__mc_rc=$?
__i=0
while [ $__i -lt 90 ]; do
	case "$(systemctl is-active b1nix-memcap.service 2>&1)" in
	active | activating) ;;
	*) break ;;
	esac
	__i=$((__i + 1))
...
__mcg=$CG/system.slice/b1nix-memcap.service
say "SYSTEMD-SMOKE:   memcap cg max=$(cat $__mcg/memory.max 2>&1) cur=$(cat $__mcg/memory.current 2>&1) events=[$(tr '\n' ' ' <$__mcg/memory.events 2>&1)]"
say "SYSTEMD-SMOKE:   oom_score_adj pid1=$(cat /proc/1/oom_score_adj 2>&1) self=$(cat /proc/self/oom_score_adj 2>&1)"
__code=$(systemctl show -p ExecMainCode --value b1nix-memcap.service 2>&1)
__status=$(systemctl show -p ExecMainStatus --value b1nix-memcap.service 2>&1)
__res=$(systemctl show -p Result --value b1nix-memcap.service 2>&1)
__pid1=$(tr -d '\000' </proc/1/comm 2>/dev/null | tr -d ' \n')
say "SYSTEMD-SMOKE:   memcap rc=$__mc_rc waited=${__i}s code=$__code status=$__status result=$__res"
# ExecMainCode 2 is CLD_KILLED; status 9 is SIGKILL. Anything else means the
# limit did not stop it -- including a clean exit, which is what a kernel with
# no memory controller produces.
if [ "$__code" = "2" ] && [ "$__status" = "9" ] && [ "$__pid1" = "systemd" ]; then
	say "SYSTEMD-SMOKE: ok memory-max"
else
	say "SYSTEMD-SMOKE: FAIL memory-max code=$__code status=$__status pid1='$__pid1'"
fi
systemctl reset-failed b1nix-memcap.service >/dev/null 2>&1

# CPUWeight between two transient units, measured from the cgroups' own
# cpu.stat rather than from ps: the number systemd's model is written in.
__psi0=$(sed -n 's/^some .*total=//p' /proc/pressure/cpu 2>/dev/null)
for __u in b1nix-cpulight b1nix-cpuheavy; do
	systemctl reset-failed "$__u.service" >/dev/null 2>&1
done
# Six busy shells per unit, not one. With one each there are two runnable
# tasks on a two-CPU machine, both get a CPU, and no weight on earth can
# express itself -- the measurement would say one to one however the units
# were configured. The weight is only visible when the machine is oversold.
__spin="for i in 1 2 3 4 5 6; do (while : ; do : ; done) & done; wait"
timeout 30 systemd-run --unit=b1nix-cpulight.service -p CPUWeight=100 \
	-p CPUAccounting=yes /bin/sh -c "$__spin" >/dev/null 2>&1
timeout 30 systemd-run --unit=b1nix-cpuheavy.service -p CPUWeight=1000 \
	-p CPUAccounting=yes /bin/sh -c "$__spin" >/dev/null 2>&1
sleep 2
__cgl=$CG/system.slice/b1nix-cpulight.service
__cgh=$CG/system.slice/b1nix-cpuheavy.service
__l0=$(awk '/^usage_usec/{print $2}' "$__cgl/cpu.stat" 2>/dev/null)
__h0=$(awk '/^usage_usec/{print $2}' "$__cgh/cpu.stat" 2>/dev/null)
sleep 6
__l1=$(awk '/^usage_usec/{print $2}' "$__cgl/cpu.stat" 2>/dev/null)
__h1=$(awk '/^usage_usec/{print $2}' "$__cgh/cpu.stat" 2>/dev/null)
systemctl stop b1nix-cpulight.service b1nix-cpuheavy.service >/dev/null 2>&1
__psi1=$(sed -n 's/^some .*total=//p' /proc/pressure/cpu 2>/dev/null)
if [ -n "$__l0" ] && [ -n "$__h0" ] && [ -n "$__l1" ] && [ -n "$__h1" ]; then
	__ld=$((__l1 - __l0))
	__hd=$((__h1 - __h0))
else
	__ld=-1
...
say "SYSTEMD-SMOKE:   cpuweight 100:1000 -> ${__ld}:${__hd} usec"
if [ "$__ld" -gt 0 ] && [ "$__hd" -gt 0 ] &&
   [ $((__hd / __ld)) -ge 3 ]; then
	say "SYSTEMD-SMOKE: ok cpu-weight"
else
	say "SYSTEMD-SMOKE: FAIL cpu-weight light=$__ld heavy=$__hd usec"
	say "SYSTEMD-SMOKE:   light cpu.stat: $(tr '\n' ' ' <"$__cgl/cpu.stat" 2>&1)"
	say "SYSTEMD-SMOKE:   heavy cpu.stat: $(tr '\n' ' ' <"$__cgh/cpu.stat" 2>&1)"
fi

# PSI, over the load that just ran. /proc/pressure is what systemd-oomd and
# every monitoring agent read, and a file that never moves is worse than none.
say "SYSTEMD-SMOKE:   psi cpu some total ${__psi0:-?} -> ${__psi1:-?} us"
if [ -n "$__psi0" ] && [ -n "$__psi1" ] && [ "$__psi1" -gt "$__psi0" ] 2>/dev/null &&
   grep -q '^some avg10=' /proc/pressure/memory 2>/dev/null &&
   grep -q '^full avg10=' /proc/pressure/io 2>/dev/null; then
	say "SYSTEMD-SMOKE: ok pressure"
else
