...
# so this reads both: the packages from tools/image/alpine/alpine.lock, the licence
# and description from the APKINDEX that fetch already downloads.
#
# Usage:
#   sh tools/image/alpine/licenses.sh            # markdown table, for the inventory
#   sh tools/image/alpine/licenses.sh --check    # exit 1 if any package has no licence
#
...
LOCK="$ROOT_DIR/tools/image/alpine/alpine.lock"
