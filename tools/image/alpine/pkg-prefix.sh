...
# (tools/image/alpine/alpine-ports.map) instead of ten near-identical files.
#
...
#
# Usage:  tools/image/alpine/pkg-prefix.sh <port>
set -eu

ROOT_DIR="$(cd "$(dirname "$0")/../../.." && pwd)"
ARCH="${B1NIX_ARCH:-${ARCH:-x86_64}}"
MAP="$ROOT_DIR/tools/image/alpine/alpine-ports.map"
...
ARCH="$ARCH" "$ROOT_DIR/tools/image/alpine/alpine-fetch.sh" "$PREFIX" $PKGS >&2
