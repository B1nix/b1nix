...
xxd -i -n vfs_b1cc_m34_runner "$ROOT/build/$ARCH/programs/bin/b1cc_m34_corpus" >> "$OUT"
xxd -i -n vfs_b1cc_m34_elf "$ROOT/build/$ARCH/programs/bin/b1cc_m34" >> "$OUT"
for src in \
  ...
  m34_runtime puts m10_switch m11_globals m18_aggregates; do
    ...
    file="$ROOT/third_party/b1cc/tests/$src.c"
    xxd -i -n "$var" "$file" >> "$OUT"
done

# Package standard headers
HEADERS=$(find "$ROOT/third_party/b1cc-sysroot" -type f | grep -v "\.DS_Store" | sort)
for h in $HEADERS; do
    rel_path=${h#$ROOT/third_party/b1cc-sysroot/}
