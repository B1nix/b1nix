...
# root, expands tools/image/limine/limine.conf.in, then produces a BIOS+UEFI hybrid ISO
# and installs the BIOS boot stages into it.
#
#   tools/image/mkiso.sh --stage DIR --out ISO --arch ARCH --kernel PATH \
...
stamp_now() {
  ...
  printf 'kernel %s\n' "$(ident "$KERNEL")"
  printf 'conf %s\n' "$(ident "$ROOT_DIR/tools/image/limine/limine.conf.in")"
  printf 'self %s\n' "$(ident "$SELF")"
