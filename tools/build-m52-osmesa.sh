#!/bin/sh
# Build the M52 OSMesa demo ELF: compile userspace/bin/m52_osmesa.c against the
...
# path. B1NIX_ARCH selects the toolchain/arch.

set -eu
ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
OUT="${1:?usage: build-m52-osmesa.sh <output-elf>}"
