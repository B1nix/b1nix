...
Edit `tools/boards/rpi4/hil.conf` to set your paths and webhook URL:
...
```bash
sh tools/boards/rpi4/hil.sh
```

### Individual Subcommands:
- **Setup TFTP environment**:
  ```bash
  sh tools/boards/rpi4/hil.sh setup
  ```
- **Build & Deploy kernel image only**:
  ```bash
  sh tools/boards/rpi4/hil.sh deploy
  ```
- **Monitor UART console only**:
  ```bash
  sh tools/boards/rpi4/hil.sh uart
  ```
