#!/bin/sh
# M26 kernel self-host: build the ext4 module that lets b1nix compile its own
# kernel, in-guest, with Alpine's clang and ld.lld.
#
#   sh tools/inguest/build-selfhost-module.sh      # after `make` (x86_64)
#
...
# b1nix's ext4 does not follow symlinks: every link becomes the file it names.
