...
MESA_SRC="$ROOT_DIR/build/src/mesa/mesa-${MESA_VERSION:-24.0.9}"
...
"$CC_CROSS" --target="$B1NIX_TRIPLET" -O2 -fPIC -ffunction-sections -fdata-sections -Db1nix \
  ...
  -nostdinc ${CC_RES:+-isystem "$CC_RES/include"} -isystem "$ROOT_DIR/build/$B1NIX_ARCH/ports/musl/install/include" -idirafter "$ROOT_DIR/userspace/include" \
