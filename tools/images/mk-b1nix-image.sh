...
# the two.
#
#   sh tools/images/mk-b1nix-image.sh                 # build/x86_64/b1nix-disk.img
#   PROFILE=broken sh tools/images/mk-b1nix-image.sh  # same, with a kernel that fails
#
# Runs as an ORDINARY USER, like everything else in this tree: the root tree is
# assembled in the user-namespace chroot from tools/packages/debian-chroot.sh,
...
CHROOT="$ROOT_DIR/tools/packages/debian-chroot.sh"
...
ls "$REPO"/dists/"$SUITE"/Release >/dev/null 2>&1 ||
	die "no repository at $REPO -- run tools/packages/build-deb.sh and tools/packages/publish-repo.sh"
...
cat >"$ROOTFS/etc/default/b1nix-boot" <<-EOF
	# Read by b1nix-update-bootloader. Edit here, not in /boot/limine.conf.
