...
# project, so it follows the same cross-build pattern as tools/build-dropbear.sh
# and tools/build-curl.sh: the b1nix-autotools-cc wrapper compiles+links against
...
if [ ! -d "$SRC_DIR" ]; then
  ...
  if [ ! -f "$tmp" ]; then
    if command -v curl >/dev/null 2>&1; then
      ...
      echo "tools/build-bash.sh: need host curl or wget to fetch $BASH_URL" >&2
      exit 1
    fi
  fi
...
# process-status type. Scoped here (not in the global libc headers) so it does
# not perturb other ports' feature detection.
