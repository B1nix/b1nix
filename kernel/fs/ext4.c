...
static struct vfs_node *ext4_vfs_mount_cb(const char *source, u64 flags, void *data) {
    ...
    if (fs->sb.s_feature_compat & EXT3_FEATURE_COMPAT_HAS_JOURNAL) {
        ...
        if (fs->journal_inum != 0) {
            ...
            if (fs->jdev) {
                fs->jdev->fs_priv = fs;
                if (fs->sb.s_feature_incompat & EXT3_FEATURE_INCOMPAT_RECOVER) { console_write("ext4: journal needs recovery\n"); journal_recover(fs->jdev); }
            }
        }
    }
    kfree(sb_buf);
...
static struct vfs_fs ext4_vfs = { .name = "ext4", .mount = ext4_vfs_mount_cb };

void ext4_init(void) {
    vfs_register_fs(&ext4_vfs);
    vfs_mount("virtio-blk0", "/ext4", "ext4", 0);
}
