...
static const char initramfs_sshd_service[] =
    ...
    /* Dropbear daemon mode: dropbear forks and backgrounds itself.
     * Retrieve PID via busybox pidof (not $!, which is unreliable in
     * the in-kernel shell) and write it to the pidfile.  No pipelines
     * or $(…) — use 'read pid < file' instead. */
