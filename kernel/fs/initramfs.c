...
static const char initramfs_bpkg_smoke[] =
    ...
    "if bpkg update >/dev/null 2>&1 && [ -f /var/lib/bpkg/index ]; then\n"
    ...
    "	&& [ \"$(cat /var/lib/bpkg/installed/hello.ver)\" = \"1.0\" ]; then\n"
    ...
    "else\n"
    "	if [ ! -f \"$ROOT/bin/badfile\" ] && [ ! -f /var/lib/bpkg/installed/badpkg.ver ]; then\n"
    ...
    "	&& [ ! -f \"$ROOT/bin/hello\" ] \\\n"
    "	&& [ ! -f /var/lib/bpkg/installed/hello.ver ]; then\n"
