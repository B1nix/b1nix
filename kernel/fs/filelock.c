...
{
    if (!filelock_initialized || fd < 0) return -1;
    ...
    if (fl->l_type == F_UNLCK) {
        ...
        for (int i = 0; i < MAX_FILE_LOCKS; i++) {
            if (file_locks[i].active && file_locks[i].pid == fl->l_pid &&
                lock_overlaps(file_locks[i].start, file_locks[i].len, fl->l_start, fl->l_len)) {
