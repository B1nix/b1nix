...
static usize iou_ring_bytes(u32 sq_entries, u32 cq_entries, u32 *cqes_off) {
  usize off = IOU_OFF_SQ_ARRAY + (usize)sq_entries * sizeof(u32);

  off = (off + 7u) & ~(usize)7u;
  if (cqes_off)
    *cqes_off = (u32)off;
  return off + (usize)cq_entries * sizeof(struct io_uring_cqe);
}
...
static void iou_flush_overflow_locked(struct io_ring_ctx *ctx) {
  while (ctx->ovfl_head) {
    ...
    if (!ctx->ovfl_head)
      ctx->ovfl_tail = 0;
    ctx->cqes[tail & ctx->cq_mask] = o->cqe;
...
static i32 iou_perform(struct iou_req *req) {
...
static void iou_complete(struct iou_req *req, i32 res, u32 cflags) {
  ...
  if (!skip)
    iou_post_req_cqe(req, res, cflags);
...
static int iou_issue(struct iou_req *req) {
  ...
  iou_complete(req, iou_perform(req), 0);
  return 0;
}
...
static int iou_progress(struct io_ring_ctx *ctx) {
  ...
  for (;;) {
    ...
    /* Ready: mark it running so the sweep does not pick it again, perform it
     * outside the lock, and complete. */
    victim->state = IOU_ST_QUEUED;
    iou_complete(victim, iou_perform(victim), 0);
  }
...
#define IOU_SQE_VALID_FLAGS                                                    \
  (IOSQE_FIXED_FILE | IOSQE_IO_LINK | IOSQE_IO_HARDLINK | IOSQE_ASYNC |        \
   IOSQE_CQE_SKIP_SUCCESS | IOSQE_IO_DRAIN)
...
static int iou_submit_one(struct io_ring_ctx *ctx,
  ...
  if (sqe->flags & ~(u8)IOU_SQE_VALID_FLAGS) {
    ...
    /* Provided-buffer rings do not exist here. Refusing the SQE is better than
     * reading into a buffer nobody chose. */
    req->failed = 1;
...
static isize iou_setup(u32 entries, u64 uparams) {
  ...
  ring_bytes = iou_ring_bytes(sq_entries, cq_entries, &cqes_off);
  ctx->ring_phys = iou_alloc_region(ring_bytes, &ctx->ring_pages);
  if (!ctx->ring_phys) {
    kfree(ctx);
    return -ENOMEM;
  }
  ctx->sqes_phys = iou_alloc_region((usize)sq_entries *
  ...
  ctx->sq_array = (u32 *)((char *)ctx->hdr + IOU_OFF_SQ_ARRAY);
...
static isize iou_register(int fd, u32 opcode, u64 arg, u32 nr_args) {
  ...
  case IORING_REGISTER_ENABLE_RINGS:
    ...
    break;
  default:
    /* Personalities, restrictions, provided-buffer rings, NAPI, resizing,
     * memory regions, io-wq tuning: none of them exist here. -EINVAL is what
     * a kernel without the opcode answers, which is what liburing tests for. */
    rc = -EINVAL;
    break;
  }
