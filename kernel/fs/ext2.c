...
static void ext2_read_bgd(u32 group, struct ext2_block_group_desc *bgd)
{
	...
	memcpy(bgd, bg_buffer + (bg_offset % ext2_block_size), sizeof(struct ext2_block_group_desc));
	kfree(bg_buffer);
}
...
static void ext2_write_bgd(u32 group, struct ext2_block_group_desc *bgd)
{
	...
	u8 *bg_buffer = kmalloc(ext2_block_size);
	ext2_read_block(bg_block, bg_buffer);
	memcpy(bg_buffer + (bg_offset % ext2_block_size), bgd, sizeof(struct ext2_block_group_desc));
...
static int ext2_read_inode(u32 inode_num, struct ext2_inode *inode)
{
	...
	memcpy(inode, inode_buffer + (inode_offset % ext2_block_size), sizeof(struct ext2_inode));
	kfree(inode_buffer);
	return 0;
}
...
static int ext2_write_inode(u32 inode_num, const struct ext2_inode *inode)
{
	...
	memcpy(inode_buffer + (inode_offset % ext2_block_size), inode, sizeof(struct ext2_inode));
...
{
	u32 groups = (ext2_sb.s_blocks_count + ext2_sb.s_blocks_per_group - 1) / ext2_sb.s_blocks_per_group;
		...
		if (bgd.bg_free_blocks_count > 0) {
			...
			for (u32 i = 0; i < ext2_sb.s_blocks_per_group; i++) {
				if (!(bitmap[i / 8] & (1 << (i % 8)))) {
					...
					u32 block_num = g * ext2_sb.s_blocks_per_group + i + (ext2_sb.s_log_block_size == 0 ? 1 : 0);
...
{
	u32 groups = (ext2_sb.s_inodes_count + ext2_inodes_per_group - 1) / ext2_inodes_per_group;
	for (u32 g = 0; g < groups; g++) {
		struct ext2_block_group_desc bgd;
		ext2_read_bgd(g, &bgd);
...
static isize ext2_vfs_read(struct vfs_node *node, u64 offset, char *buffer, usize size)
{
	...
	while (bytes_read < to_read) {
		...
		if (phys_block == 0) {
			// Sparse block or unsupported indirect
...
static isize ext2_vfs_write(struct vfs_node *node, u64 offset, const char *buffer, usize size)
{
	u32 inode_num = (u32)(usize)node->data;
	struct ext2_inode inode;
	if (ext2_read_inode(inode_num, &inode) < 0) return -EIO;
	...
	if (new_size > inode.i_size) {
		u32 old_blocks = (inode.i_size + ext2_block_size - 1) / ext2_block_size;
		u32 new_blocks = (new_size + ext2_block_size - 1) / ext2_block_size;
		
		for (u32 b = old_blocks; b < new_blocks; b++) {
...
static int ext2_add_dir_entry(u32 dir_inode_num, u32 inode_num, const char *name, u8 type)
{
	...
	if (ext2_read_inode(dir_inode_num, &dir_inode) < 0) return -1;
	...
	if (blocks == 0) {
		...
		if (!new_phys) { kfree(dir_buf); return -1; }
	...
	for (u32 b = 0; b < blocks; b++) {
		...
		while (offset < ext2_block_size) {
			...
			if (entry->rec_len >= actual_len + needed_len) {
				...
				struct ext2_dir_entry *new_entry = (struct ext2_dir_entry *)(dir_buf + offset + actual_len);
				new_entry->inode = inode_num;
...
static int ext2_vfs_create(struct vfs_node *dir, const char *name, const char *full_path)
{
	...
	if (ext2_add_dir_entry(dir_inode_num, new_inode_num, name, EXT2_FT_REG_FILE) < 0) {
		return -1;
	}
	
	struct vfs_node *node = vfs_add_node(full_path, VFS_FILE, (void *)(usize)new_inode_num, 0, 0);
	if (node) {
...
static void ext2_populate_vfs(u32 inode_num, const char *base_path)
{
	...
	for (u32 i = 0; i < (inode.i_size + ext2_block_size - 1) / ext2_block_size; i++) {
			...
			if (strcmp(name, ".") != 0 && strcmp(name, "..") != 0) {
				...
				if (ext2_read_inode(entry->inode, &child_inode) == 0) {
					if ((child_inode.i_mode & EXT2_S_IFMT) == EXT2_S_IFDIR) {
						struct vfs_node *dir_node = vfs_add_node(full_path, VFS_DIRECTORY, (void *)(usize)entry->inode, 0, 0);
						if (dir_node) dir_node->create_cb = ext2_vfs_create;
						ext2_populate_vfs(entry->inode, full_path);
					} else {
						struct vfs_node *node = vfs_add_node(full_path, VFS_FILE, (void *)(usize)entry->inode, child_inode.i_size, 0);
						if (node) {
							node->read_cb = ext2_vfs_read;
							node->write_cb = ext2_vfs_write;
						}
