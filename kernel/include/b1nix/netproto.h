...
struct net_proto {
   ...
   * was sent and the caller should retry. `dev` is the egress interface the
   * route lookup chose (M84 multi-NIC routing) — resolving on the wrong link
   * would solicit a neighbour that is not on it. NULL means "the default
   * interface", which is what a single-NIC system always passes. */
