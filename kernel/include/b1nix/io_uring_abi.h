...
/*
 * The io_uring user ABI, taken verbatim from Linux 6.18.51's
 * include/uapi/linux/io_uring.h — the same kernel version the imported
 * filesystems are pinned to (tools/fs/fetch-linux-fs.sh).
 *
 * It is vendored rather than retyped on purpose. liburing is compiled against
 * these exact structures, and the ring head/tail protocol, the SQE layout and
 * every flag value are what a program tests for. A near-miss is worse than an
 * absence, and a hand-copied 64-byte union with one field in the wrong place
 * is a class of bug no test names. The file is dual licensed
 * (GPL-2.0 WITH Linux-syscall-note) OR MIT upstream; MIT is taken here.
 *
 * The ONLY edits are the three #includes at the top, replaced by the fixed
 * width types below, because this file is compiled inside a kernel that has no
 * linux/types.h of its own on the b1nix side. Nothing else may be changed: if
 * a structure here disagrees with Linux, that is the bug.
 *
 * What b1nix actually implements out of this ABI is a subset, and the subset is
 * enforced in one place — io_uring_setup() refuses a flag it cannot honour and
 * an unimplemented opcode completes with -EINVAL. See kernel/fs/io_uring.c.
 */
#ifndef B1NIX_IO_URING_ABI_H
#define B1NIX_IO_URING_ABI_H
...
/* linux/types.h: an 8-byte-aligned __u64, which matters only on 32-bit
 * architectures. b1nix is 64-bit on both targets, so the alignment is already
 * what the attribute asks for. */
typedef u64 __aligned_u64;

/* linux/stddef.h. A flexible array inside a union needs a struct of its own,
 * which is all the macro does. */
