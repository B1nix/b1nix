...
/* perf_event(2)'s user ABI, taken verbatim from Linux's
 * include/uapi/linux/perf_event.h (6.18-era) rather than retyped.
 *
 * `perf` is compiled against these structures: struct perf_event_attr's field
 * order and size, the bitfield layout of the flags word, the sample-record
 * formats and the mmap'd control page are all read by the distribution's own
 * binary, so a near-miss is worse than an absence. The only edits are the three
 * host includes, replaced by the type shims below, and the ioctl numbers, which
 * are spelled out because b1nix has no _IO/_IOW macros.
 *
 * What of this ABI is actually implemented is decided in kernel/perf/
 * perf_event.c and nowhere else; the header is only the shape.
 */
#ifndef B1NIX_PERF_EVENT_ABI_H
#define B1NIX_PERF_EVENT_ABI_H
...
/* The ioctl numbers, as _IO/_IOW/_IOR expand them on Linux. */
...
#define PERF_EVENT_IOC_SET_FILTER   0x40082406u
#define PERF_EVENT_IOC_ID           0x80082407u
#define PERF_EVENT_IOC_SET_BPF      0x40042408u
#define PERF_EVENT_IOC_PAUSE_OUTPUT 0x40042409u
#define PERF_EVENT_IOC_QUERY_BPF    0xc008240au
#define PERF_EVENT_IOC_MODIFY_ATTRIBUTES 0x4008240bu
