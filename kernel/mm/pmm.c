...
static usize frame_index(u64 frame)
{
	return (usize)(frame / PAGE_SIZE);
}

static u64 frame_from_index(usize index)
{
	return (u64)index * PAGE_SIZE;
}

static int bitmap_get(usize index)
{
	return (pmm.bitmap[index / BITS_PER_BYTE] & (1u << (index % BITS_PER_BYTE))) != 0;
}
...
static u64 find_bitmap_address(const struct boot_info *boot_info, usize bitmap_bytes)
{
...
void pmm_init(const struct boot_info *boot_info)
{
	...
	for (u64 frame = pmm.kernel_start; frame < pmm.kernel_end; frame += PAGE_SIZE) {
	...
	for (u64 frame = (u64)(usize)pmm.bitmap; frame < (u64)(usize)pmm.bitmap + pmm.bitmap_bytes; frame += PAGE_SIZE) {
		mark_frame_used(frame);
	}
