...
static int ahci_port_read(struct ahci_port_state *port, u64 lba, u32 count, void *buffer)
{
    ...
    prdt->dbc = (count * 512 - 1) | (1 << 31); // byte count, I=1 (interrupt on completion)
...
static int ahci_port_write(struct ahci_port_state *port, u64 lba, u32 count, const void *buffer)
{
    if (!port->present) return -1;

    volatile struct ahci_port *p = &port->abar->ports[port->port_num];
    struct ahci_cmd_header *cmd_hdr = &port->cmd_list[1];  // Use slot 1 for writes
    struct ahci_cmd_table *cmd_table = (struct ahci_cmd_table *)((u8 *)port->cmd_table + 4096); // Separate table for slot 1
...
static int ahci_blk_read(struct block_device *dev, u64 lba, u32 count, void *buffer)
{
    struct ahci_port_state *port = (struct ahci_port_state *)dev->priv;
    return ahci_port_read(port, lba, count, buffer);
}

static int ahci_blk_write(struct block_device *dev, u64 lba, u32 count, const void *buffer)
{
    struct ahci_port_state *port = (struct ahci_port_state *)dev->priv;
    return ahci_port_write(port, lba, count, buffer);
}

static void ahci_port_init(struct ahci_port_state *port, volatile struct ahci_hba_mem *abar, int num)
{
    port->abar = abar;
    port->port_num = num;
    port->present = 0;
    ...
    // Allocate FIS receive area (256 bytes, must be 256-byte aligned, but page is fine)
    ...
    // ctba = lower 32 bits, ctbau = upper 32 bits (actually ctba=slot0, ctba+16=slot1)
    port->cmd_list[0].ctba = (u32)(port->phys_cmd_table & 0xFFFFFFFF);
    port->cmd_list[0].ctbau = (u32)((port->phys_cmd_table >> 32) & 0xFFFFFFFF);
    port->cmd_list[1].ctba = (u32)((port->phys_cmd_table + PAGE_SIZE) & 0xFFFFFFFF);
    port->cmd_list[1].ctbau = (u32)(((port->phys_cmd_table + PAGE_SIZE) >> 32) & 0xFFFFFFFF);
            ...
            for (u16 bus = 0; bus < 256 && !found; bus++) {
                for (u8 slot = 0; slot < 32 && !found; slot++) {
                    ...
                    if (class == 0x01) {
                        ...
                        if (pci.vendor_id == 0xFFFF) continue;
                        pci.device_id = pci_config_read16((u8)bus, slot, 0, 2);
                        pci.class_code = class;
