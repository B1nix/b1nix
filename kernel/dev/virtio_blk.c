...
static int do_virtio_blk_req(struct virtio_blk_instance *inst, u64 lba, u32 count, void *buffer, u32 type)
{
	...
	inst->vq.desc[d1].flags = VRING_DESC_F_NEXT | (type == VIRTIO_BLK_T_IN ? VRING_DESC_F_WRITE : 0);
...
{
	struct virtio_blk_instance *inst = (struct virtio_blk_instance *)dev->priv;
	return do_virtio_blk_req(inst, lba, count, (void *)(usize)buffer, VIRTIO_BLK_T_OUT);
}
...
void virtio_blk_init(void)
{
	...
	while (instance_count < MAX_VIRTIO_BLK) {
		...
		if (!virtio_find_nth_device(dev_idx, &pci_info)) break;
		...
		inst->dev.irq = pci_config_read8(pci_info.bus, pci_info.slot, pci_info.func, 0x3C);

		// Reset device
		virtio_set_status(&inst->dev, 0);
		virtio_set_status(&inst->dev, VIRTIO_STATUS_ACKNOWLEDGE | VIRTIO_STATUS_DRIVER);
		virtio_set_guest_features(&inst->dev, 0);
		virtio_set_status(&inst->dev, VIRTIO_STATUS_ACKNOWLEDGE | VIRTIO_STATUS_DRIVER | VIRTIO_STATUS_FEATURES_OK);
		...
		virtio_set_status(&inst->dev, virtio_get_status(&inst->dev) | VIRTIO_STATUS_DRIVER_OK);
		...
		name[8] = 'l'; name[9] = 'k'; name[10] = '0' + (char)instance_count;
		name[11] = '\0';
