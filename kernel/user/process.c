...
static int copy_string_vector(const char **src, int max_count, const char ***out, int *out_count)
{
	...
	if (src) {
		for (; count < max_count; count++) {
			...
			if (syscall_copyin(&user_string, src + count, sizeof(user_string)) != 0) return -1;
			if (!user_string) break;
			if (syscall_copyinstr(tmp, sizeof(tmp), user_string) != 0) return -1;
			copy[count] = kernel_strdup(tmp);
			if (!copy[count]) return -1;
		}
	}
	copy[count] = 0;
	*out = copy;
...
static int user_build_initial_stack(struct user_loaded_image *image)
{
	...
	user_stack_push_u64(stack, &sp, 3); /* AT_PHDR, populated virtually for debuggers */
...
static int user_image_read_vfs_file(const char *path, char **out_data, usize *out_size)
{
	struct vfs_node *node = vfs_find_node(path);
	if (!node || node->type != VFS_FILE || node->size == 0) return -1;
	...
	vfs_close(fd);
	if (got < 0 || (usize)got != node->size) return -1;
...
static int user_load_elf64(struct user_loaded_image *image, const char *path)
{
	char *file_data = 0;
	usize file_size = 0;
	if (user_image_read_vfs_file(path, &file_data, &file_size) != 0) return -1;
	if (file_size < sizeof(struct elf64_ehdr)) return -1;
	...
	if (ehdr->e_ident[4] != ELF_CLASS_64 || ehdr->e_ident[5] != ELF_DATA_LE) return -1;
	if (ehdr->e_type != ELF_TYPE_EXEC) return -1;
	if (ehdr->e_machine != ELF_MACHINE_X86_64 && ehdr->e_machine != ELF_MACHINE_AARCH64) return -1;
	if (ehdr->e_phoff + ((u64)ehdr->e_phentsize * ehdr->e_phnum) > file_size) return -1;
	...
	for (u16 i = 0; i < ehdr->e_phnum; i++) {
		struct elf64_phdr *phdr = (struct elf64_phdr *)(file_data + ehdr->e_phoff + ((u64)i * ehdr->e_phentsize));
		if (phdr->p_type != PT_LOAD) continue;
		if (image->segment_count >= USER_MAX_IMAGE_SEGMENTS) return -1;
		if (phdr->p_offset + phdr->p_filesz > file_size || phdr->p_filesz > phdr->p_memsz) return -1;

		struct user_image_segment *segment = &image->segments[image->segment_count++];
...
static struct user_loaded_image *user_load_image(const char *path, int argc, const char **argv, const char **envp)
{
	struct user_loaded_image *image = kzalloc(sizeof(*image));
	if (!image) return 0;

	if (copy_string_vector(argv, USER_MAX_ARGS, &image->argv, &image->argc) != 0) return 0;
	if (argc > 0 && image->argc > argc) image->argc = argc;
	if (copy_string_vector(envp, USER_MAX_ENVS, &image->envp, 0) != 0) return 0;
	...
	if (user_load_elf64(image, path) == 0) {
		if (user_build_initial_stack(image) != 0) return 0;
		return image;
	}
...
static void user_process_thread(void *arg)
{
	...
	console_write_hex64(image->address_space.pml4_frame);
...
int user_spawn(const char *path, int argc, const char **argv)
{
	...
	struct user_loaded_image *image = user_load_image(path, argc, argv, empty_env);
	if (!image) {
...
int user_execve_current(const char *path, const char **argv, const char **envp)
{
	...
	console_write("user: execve image ");
	console_write(path);
	console_write("\n");
