...
/* Smoke-mode screen redirection: point fd 1 at /dev/null so a full-screen
 * repaint doesn't flood the serial console (which is also the smoke log —
 * tens of KB of escape sequences per app run push the whole suite past its
 * serial-silence watchdog). The write() calls still happen, so the render
 * path is exercised exactly as in a real run; only the bytes are discarded.
 * tui_screen_unmute() restores the original fd 1 for marker output. */
