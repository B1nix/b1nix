...
/* b1nix: NetSurf's amalgamation injected an unconditional
 * `#include "duk_custom.h"` here. That custom header pulls in NetSurf-internal
 * headers (utils/config.h, netsurf/inttypes.h) and a browser-specific execution
 * timeout hook, none of which exist for the standalone /bin/js. Removed so the
 * default Duktape configuration applies for the standalone interpreter. */
