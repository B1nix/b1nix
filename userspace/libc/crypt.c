/* M32b: userspace password hashing — SHA-512 + the b1nix "$b1$" crypt scheme,
 * mirroring kernel/lib/sha512.c + kernel/lib/crypt.c so userspace daemons
 * (dropbear) can verify passwords against /etc/shadow exactly like the in-kernel
 * login built-in does.
 *
 *     h0 = sha512(salt || ":" || password)
 *     h_{i+1} = sha512(h_i || salt)   for i in [0..1023)
 *     out = "$b1$<salt>$<base64(h_1024)>"
 *
 * Provides POSIX crypt(key, setting): parses the salt out of `setting` and
 * returns the full "$b1$<salt>$<hash>" string in a static buffer. */
...
static void sha512_update(struct sha512_ctx *c, const void *data, size_t len) {
  ...
  while (len > 0) {
    uint32_t take = 128 - c->buflen;
    if (take > len) take = (uint32_t)len;
    memcpy(c->buf + c->buflen, p, take);
    c->buflen += take; p += take; len -= take;
    if (c->buflen == 128) { sha512_transform(c, c->buf); c->buflen = 0; }
  }
}
...
static void sha512_final(struct sha512_ctx *c, uint8_t out[64]) {
  ...
  if (c->buflen > 112) {
    ...
    sha512_transform(c, c->buf); c->buflen = 0;
  }
  while (c->buflen < 112) c->buf[c->buflen++] = 0;
  for (int i = 0; i < 8; i++) c->buf[112 + i] = 0;
  for (int i = 0; i < 8; i++) c->buf[120 + i] = (uint8_t)(c->bitlen >> (56 - i * 8));
...
/* POSIX crypt(): `setting` is a stored "$b1$<salt>$<hash>" (or just "$b1$<salt>$").
 * Returns "$b1$<salt>$<hash-of-key>" in a static buffer, or NULL on bad input. */
char *crypt(const char *key, const char *setting) {
  static char result[160];
  if (!key || !setting) return NULL;
  if (strncmp(setting, "$b1$", 4) != 0) return NULL;

  const char *salt_start = setting + 4;
  const char *salt_end = strchr(salt_start, '$');
  if (!salt_end) return NULL;
  size_t slen = (size_t)(salt_end - salt_start);
  if (slen == 0 || slen > 32) return NULL;

  char salt[33];
  memcpy(salt, salt_start, slen);
  salt[slen] = '\0';

  if (b1nix_crypt(key, salt, result, sizeof(result)) != 0)
    return NULL;
  return result;
}
