...
- [ ] `partial` The io_uring remainder: `RECV_ZC`, io-wq affinity, NAPI busy-poll, buffer cloning, ring resizing, memory regions and the query interface, and a `SEND_ZC` that pins the caller's pages instead of copying. What liburing's own suite found on the way is fixed: the registered file resolved at issue rather than at prep, the direct close by slot, the armed connect reporting its real result, and futex waits that queue for a wake instead of polling the word (see [processes-and-system-calls.md](processes-and-system-calls.md)).
