...
- [x] `partial` `perf_event_open`: the software counters this kernel really keeps (task clock, CPU clock, page faults with the minor/major split, context switches, `DUMMY`), timer-driven sampling with user call chains walked without faulting, and the mmap'd ring buffer with `PERF_RECORD_SAMPLE`, `COMM`, `MMAP2`, `EXIT` and the `sample_id_all` trailer. `/proc/sys/kernel/perf_event_paranoid` decides what an unprivileged caller may profile. **PMU hardware counters are not implemented** — `PERF_TYPE_HARDWARE`, `HW_CACHE`, `RAW` and `TRACEPOINT` are `EOPNOTSUPP` rather than answered with zero, and `perf record -e cycles` therefore does not work.
...
- [ ] `planned` Replace ad-hoc profiling hooks (`kprof`, the `b1nix.sysprof` histogram) where `perf` now answers the same question.
- [x] `done` Proof: 18 checks in the posix lane (`m126_smoke`) open the counters by hand and make the thing each one counts happen — 120 ms of spinning moves the task clock, 256 fresh pages move the fault counter, twenty sleeps move the switch counter — then map the ring buffer, take 300 ms of samples at 200 Hz and walk the records: every sample carries an instruction pointer, the call chains start in the sampled program, `poll(2)` reports the buffer readable, and `IOC_REFRESH(3)` delivers exactly three. `perf record` from the distribution has **not** been run.
