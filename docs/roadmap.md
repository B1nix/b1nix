...
- [ ] `partial` Add IPv6 networking. **Done (phase 2, 2026-06-01):** kernel IPv6 datapath (`kernel/net/ipv6.c`) with an IPv6 header parser, an ICMPv6 echo responder, and a `::1` loopback fast path mirroring the IPv4 one; pseudo-header-checksummed ICMPv6; a `ping ::1` kernel self-test that round-trips an echo request/reply (`M32-IP6: ok icmpv6-loopback`); and DNS AAAA-record parsing in `kernel/net/dns.c` (`dns_last_result6`) verified by an offline synthesized response (`DNS-SMOKE: ok parse-aaaa-record`). The libc/userspace side (`AF_INET6`, `sockaddr_in6`, numeric `getaddrinfo` for `::1`) already landed in phase 1. **Still TODO:** neighbor discovery, off-link routing, real-link (ethertype 0x86DD) send, UDP/TCP over IPv6 through the socket layer (`tcp.c`/`udp.c`/`socket.c` are still IPv4-only), dual-stack socket behavior, and `getnameinfo`.
