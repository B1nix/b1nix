...
- [x] `partial` Audit and remove the Big Kernel Lock (BKL) for userspace execution on APs. Foundation done: lock-order DAG in [`docs/m28-locking.md`](m28-locking.md), lockdep tracker (`-DKERNEL_LOCKDEP=1`) with Variant-A bequeath relaxation (`lockdep_*_global` for BKL + per-inode rw_lock — no false positives on cross-CPU release), TLB shootdown IPI (0x41, gated), reschedule IPI (0x42) wired into `wake_sleepers` / `scheduler_wake_all` / `scheduler_exit_current` (F6). Atomic-CAS state transitions: DEAD→REAPING (F1), SLEEPING/BLOCKED→READY (F4), READY→RUNNING in `pick_next_task` (F5), BLOCKED→READY in exit_current's parent wake (F6). `bkl_unlock` is a no-op for non-owner CPUs. **Done: T1 (AP idle outer loop) + T2 (BSP idle) + T3 (LAPIC timer ISR)**. **Primary T4 stack-lifetime race FIXED in tree as a strict improvement**: per-task `stack_released` lease — `arch_context_switch` takes 3rd arg `&old->stack_released` and stores `1` after RSP swap; `scheduler_exit_current` + signal SIGKILL/SIGDFL terminators + `ap_worker_trampoline` clear `0` before `state = TASK_DEAD`; `scheduler_waitpid` polls `==1` before `kfree(stack)`. This closes the M14 `iretq` GP fault deterministically. **T4 (`bkl_lock`/`bkl_unlock` removal from syscall_entry.S) was applied, characterized at 60% pass on smoke (vs documented ~67% baseline flake rate), then reverted**: residual ~40% failures don't have fork/exec shape (kernel-heap RIPs, memmove crashes — look like heap UAF / spurious stack-lifetime issues in other paths). Two stabilizer attempts also reverted: BKL-on-fork-only (CPU-bequeath model breaks under partial coverage → 0%), task-owned sleeping mutex (correct in theory, ~19x slowdown in practice). T4 stabilization needs targeted debugging of specific residual shapes — out of scope for this branch. See [`docs/m28-t4-blocker.md`](m28-t4-blocker.md) for the full story. Smoke 250/0/0 production through T1+T2+T3+T8 with race fix + lockdep Variant-A in tree.
