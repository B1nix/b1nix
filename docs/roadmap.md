...
- [x] `done` Close M24b with ordinary userspace-on-AP execution explicitly deferred beyond this milestone. With per-CPU `current_task` now in place, the remaining piece is a focused Big Kernel Lock effort: acquire the lock on syscall/interrupt entry, release it on return-to-user / while idling, run the cooperative scheduler on APs (not just the isolated kernel-worker path in `ap_main`), and hand off so user mode can still run in parallel. Until that follow-up lands, `stealable` stays restricted to self-contained CPU-bound kernel workers — APs do not run userspace ELFs. This keeps M24b scoped to verified AP bringup, per-CPU scheduler state, task stealing, and SMP-safe task/FD bookkeeping. Future BKL work should verify `tests/smoke.sh x86` (incl. the `-smp 4` block) at each step. Note: the Makefile has no auto header-deps (`-MMD`), so changes to `sched.h`/`lapic.h` need a `make clean`.
