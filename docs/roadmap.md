...
- [ ] `partial` Add auth storage and policy: `/etc/shadow` password auth, optional `authorized_keys`, account shell/home validation, login failure accounting, and clear root-login defaults. **Done so far (2026-06-02):** libc `getpwnam`/`getpwuid` now substitute the real `/etc/shadow` hash when `/etc/passwd` has the `x` marker (`userspace/libc/pwd.c`), and a POSIX `getpass()` was added — the prerequisites dropbear's password auth needs. End-to-end auth is gated on the item-9 KEX completing.
...
- [ ] `partial` Smoke-test sshd on localhost first: SSH protocol handshake, host-key persistence, password login to run a single command, interactive shell over pty, and negative auth cases. **In progress (2026-06-02):** the localhost exchange now runs the full key exchange — banner exchange, KEXINIT algorithm negotiation (ssh-ed25519 + chacha20-poly1305) and the curve25519 ECDH packet exchange complete in both directions. Two real kernel bugs were fixed to get here: (1) `vfs_close` tore down a socket on every fd close even when it was still shared across `fork()` (the daemon's accept-fork-close pattern FIN'd the peer mid-handshake) — teardown moved to the refcount-zero `->release` op; (2) loopback TCP delivered synchronously and re-entered the TCP state machine mid-send — loopback datagrams are now deferred and drained by `net_poll`/`net_task`, which also made all loopback TCP smoke tests pass and removed a suite hang. **Open blocker:** the client receives `KEX_ECDH_REPLY` but does not yet run `recv_msg_kexdh_reply` (→ NEWKEYS → password auth → remote command), so the `M32B-SSH-LOGIN-OK` marker is not yet emitted; the smoke check is non-fatal so it cannot mask the rest of the suite. Full analysis in [`docs/m32b-ssh.md`](m32b-ssh.md).
