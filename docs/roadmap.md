...
- [ ] `partial` Add IPv6 networking. **Done (phase 2–3, 2026-06-01):** kernel IPv6 datapath (`kernel/net/ipv6.c`) with an IPv6 header parser, an ICMPv6 echo responder, and a `::1` loopback fast path mirroring the IPv4 one; pseudo-header-checksummed ICMPv6; a `ping ::1` kernel self-test that round-trips an echo request/reply (`M32-IP6: ok icmpv6-loopback`); DNS AAAA-record parsing in `kernel/net/dns.c` (`dns_last_result6`) verified by an offline synthesized response (`DNS-SMOKE: ok parse-aaaa-record`); and **UDP over IPv6 through the real socket layer** — `AF_INET6`/`struct b1nix_sockaddr_in6` in the kernel, `socket()`/`bind()`/`connect()`/`send()`/`recv()` for `AF_INET6 SOCK_DGRAM`, and a `udp6_send`/`udp6_receive` path (pseudo-header checksum) over `::1`, end-to-end smoke-verified from userspace (`M32-NET: ok udp6-loopback`). The libc/userspace side (`AF_INET6`, `sockaddr_in6`, numeric `getaddrinfo` for `::1`) landed in phase 1. **Still TODO:** TCP over IPv6 (`tcp.c` is still keyed on `struct ipv4_addr` — a real refactor), neighbor discovery, off-link routing, real-link (ethertype 0x86DD) send, dual-stack socket behavior, and `getnameinfo`.
- [x] `done` Add PCRE/PCRE2 support as an optional userspace port (2026-06-01). `tools/build-pcre2.sh` cross-builds the static 8-bit `libpcre2-8.a` (10.44) through the b1nix autotools wrapper (config.sub `b1nix*` patch + fixed-timestamp trick so make never needs autoconf/automake; `--disable-jit`, 8-bit only). The PCRE2 runtime is proven on b1nix by a standalone smoke (`/bin/m32-pcre2-smoke`) that compiles a pattern and verifies a capturing match plus a correct non-match (`M32-PCRE2: ok compile`/`ok match`/`ok nomatch`/`done`). No libc gaps were needed — `libb1nix.a` already covers PCRE2's dependencies. Wiring `--with-pcre2` into the wget/curl builds is tracked under the incremental-re-enable item below.
...
- [ ] `planned` Re-enable the disabled curl/wget configure features incrementally (`SSL`, `IPv6`, `PCRE2`, `IRI/IDN`, `NTLM`) with one smoke marker per feature instead of flipping them all at once. `PCRE2` is now buildable (see above), so wget `--with-pcre2` (its `--accept-regex`/`--reject-regex` PCRE2 mode) is the next concrete sub-step.
- [ ] `blocked` **wget TLS/HTTPS — what we still need.** Unlike curl, wget cannot use mbedTLS: GNU Wget's `configure` only accepts `--with-ssl={gnutls,openssl,no}`, so HTTPS for `/bin/wget` is blocked until one of those backends is ported to the b1nix userspace ABI. Concretely:
  - **Pick a backend.** *OpenSSL* (libssl + libcrypto) is self-contained but large and has its own `Configure`/perl-driven build that needs a `b1nix` target triplet and assembly/`OPENSSL_NO_ASM` decisions; *GnuTLS* is smaller per-library but pulls a dependency chain — **libnettle + libhogweed** (crypto), **libgmp** (bignum; or nettle's mini-gmp), **libtasn1**, and **libunistring** — each of which is its own autotools port. Recommendation: evaluate OpenSSL first (one port vs. five) unless its build proves too host-coupled, in which case fall back to GnuTLS with `--with-included-libtasn1`/`--with-included-unistring` to cut two of the deps.
