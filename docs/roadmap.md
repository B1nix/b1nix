...
- [x] `partial` (v0.90.1) **GCC-free Rust.**
  Rust now links the LLVM runtimes (`libunwind.a`/`libcompiler_rt.a`) instead of GCC/`libgcc_s`:
  - **M67 cross rust — done & smoke-verified.** The JSON target spec (`build/rust/targets/x86_64-unknown-b1nix.json`, static `crt-static` ET_EXEC) drops `-lgcc_s`; Rust's own `unwind` crate now links the LLVM libunwind static bundle, and `build-rust-toolchain.sh` stages `libunwind.a`/`libcompiler_rt.a` into the cross sysroot. `tools/m67/hello_b1nix.elf` rebuilt GCC-free (linked `-lunwind`, no `-lgcc_s`); `M67-RUST: ok run-std` green in the full suite. Also fixed three real Rust-vs-clang gaps in the M90 cross-cc wrapper (`b1nix-autotools-cc`): recognise `.rlib`/`.so` link inputs (were silently dropped), honour a driver-level `-static`, and add default sysroot `-L` search paths.
  - **M68 native rustc — code-ready, rebuild pending.** The built-in target spec (`tools/patches/rust/x86_64_unknown_b1nix.rs`), `build-rust-native.sh` (stages `libcompiler_rt.a` too) and `tools/rust/rust-proof.sh` (drops the `libgcc_s.so` bundling) are all updated to the GCC-free recipe, but regenerating the ~250 MB native `rustc` + running the `M68-RUST` proof needs a multi-hour from-source rust+LLVM rebuild (the source tree is not present on this checkout) and was not run here.
