...
- [ ] `wontfix` **BusyBox modutils** — `insmod`, `rmmod`, `lsmod`, `modprobe`, `depmod`, `modinfo` stay unbuilt, but the commands exist: M95/M96 ship real loadable modules and b1nix's own `/bin/kmod` multicall drives them. BusyBox's modutils speak Linux's `init_module`/`finit_module` ABI, which this loader does not implement, so enabling them would add commands that fail rather than commands that work.
