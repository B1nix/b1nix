...
- [x] `partial` Return large-allocation pages to the pmm (`kernel/mm/kheap.c`). Adds a separate page-granular **large-allocation arena** (`klarge_*`): every `kmalloc`/`kzalloc` ≥ `KLARGE_THRESHOLD` (256KB) is mapped from fresh pmm frames into the upper part of the kheap's `PML4[384]` slot and its whole pages are **unmapped + returned to the pmm on `kfree`** (with vaddr-span reuse). This caps the high-water growth from the big transient cc1/as/ld ELF-staging buffers (the ~516KB and ~33MB allocations that drove the in-guest kheap to ~2GB), so it tracks live large-buffer use instead of growing monotonically. The general bump heap below is left **byte-for-byte identical to the known-good baseline** (no coalescing, no boundary tags) — chosen deliberately after discovering that general-heap coalescing turns a **pre-existing latent heap UAF** (active during M25/TCC) from silent into fatal (see [`docs/m26-selfhost.md`](m26-selfhost.md) UPDATE l). Host smoke reaches `B1NIX-TEST: done` (all modules, M25/TCC + M16/mc pass), and `[KLARGE]` traces confirm `free_frames` recovers on every large free. **Not yet verified:** end-to-end 2048MB in-guest build completion (needs the Fedora/KVM rig + populated root image; mechanism is host-verified, full build pending).
- [ ] `planned` Fix the pre-existing latent heap UAF surfaced during M25/TCC (a consumer writes into an allocated block's header asynchronously of the owner). Reproduce by re-enabling the aggressive per-op `kheap_validate` walk (kept in git stash `m26-coalesce-wip`); it panics `kheap magic/prev_size corrupt` deep in M25 even with coalescing + page-return disabled (so the corruption is NOT caused by the new allocator). Leading suspects from a driver DMA audit: AHCI/NVMe completion-wait timeouts that return with the command still queued (`ahci.c:97-108`, `nvme.c:162-188`) and `virtio_blk.c:81-83` missing read barriers — all wait via `scheduler_yield()` (IRQs on) so DMA can land after the buffer is freed/reused. Fixing this is the prerequisite to safely enabling general-heap coalescing. See [`docs/m26-selfhost.md`](m26-selfhost.md) UPDATE l.
