...
- [x] `partial` Read-write lock primitive + VFS chain-walk lock (M28-B). Added a kernel-internal `rwlock_t` (header-only, atomic, IRQ-save variants — `kernel/include/b1nix/rwlock.h`) and a global `vfs_tree_lock` that explicitly protects every `vfs_node` parent/sibling chain walk in `kernel/fs/vfs.c`: readers in `find_child` + `vfs_get_mount_for_node` (replaces the implicit cli/sti window), writers in `add_node` × 2 / `vfs_create` / `vfs_mkdir` / `vfs_remove_node` / `vfs_unlink_at_internal` / `vfs_link` / `vfs_symlink` / `vfs_rename_internal` (every sibling-list mutation + rollback). The lock is held only over the actual mutation, so per-inode sleeping locks remain outside (no yield-with-IRQs-off deadlock risk). The process table + memory manager are NOT yet converted — those are the remaining pieces. Today this runs under the BKL so it is decorative; wiring it now is the per-subsystem foothold that lets a future BKL teardown remove cross-CPU races one site at a time. Smoke 250/0/0.
