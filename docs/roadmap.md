...
- [ ] `partial` Re-enable the disabled curl/wget configure features incrementally (`SSL`, `IPv6`, `PCRE2`, `IRI/IDN`, `NTLM`) with one smoke marker per feature instead of flipping them all at once. **Done so far:** curl `SSL` (mbedTLS, see above); curl `--enable-ipv6` (2026-06-01) with `curl -6` HTTP/HTTPS smoke probes (`M32-NET: ok ext-http6`/`ext-https6`, graceful offline skip). **Remaining:** wget `--with-pcre2` (PCRE2 is buildable, see above — next concrete sub-step), wget `IRI/IDN` and `NTLM` (and wget `SSL`, blocked — see below). `IRI`/`NTLM` stay `planned` per the dedicated items above.
