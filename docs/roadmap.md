...
- [x] `done` Per-CPU LAPIC scheduler tick (M28-A) + preemptive yield (T8 / M28 #8). Each core ticks itself off `LAPIC_TIMER_VECTOR` (0x40) at 100 Hz instead of the BSP-only PIT IRQ0. `scheduler_on_timer_tick` now calls `scheduler_yield()` — the long-deferred "re-enable once VFS locking is audited" step. The foundation that closes it: vfs_tree_lock IRQ-save semantics (a chain walker holds the rwlock with IRQs disabled, so same-CPU timer cannot preempt it mid-walk), every other spinlock is irq-save or per-task, and T3 removed BKL from the timer ISR so the yield isn't recursively contended. EOI moved BEFORE `scheduler_on_timer_tick` so the LAPIC unblocks immediately and isn't wedged when the yield context-switches away. Vector 32 (PIT) kept as a calibration-failed fallback; vector 255 (LAPIC spurious) wired as a no-EOI no-op. Smoke 250/0/0 single CPU + `-smp 4`.
- [x] `partial` Audit and remove the Big Kernel Lock (BKL) for userspace execution on APs. Foundation done: lock-order DAG in [`docs/m28-locking.md`](m28-locking.md), lockdep tracker (`-DKERNEL_LOCKDEP=1`), TLB shootdown IPI (0x41, gated), reschedule IPI (0x42) wired into `wake_sleepers` / `scheduler_wake_all` / `scheduler_exit_current` (F6). Atomic-CAS state transitions: DEAD→REAPING (F1), SLEEPING/BLOCKED→READY (F4), READY→RUNNING in `pick_next_task` (F5), BLOCKED→READY in exit_current's parent wake (F6). `bkl_unlock` is a no-op for non-owner CPUs. **Done: T1 (AP idle outer loop) + T2 (BSP idle) + T3 (LAPIC timer ISR)**. Each Tn keeps `bkl_unlock()` before sti;hlt to release BKL bequeathed by syscall-return. T3 also fixed two pre-existing bugs: (a) `scheduler_sleep_ticks` / `scheduler_block_on` / `scheduler_block_current` now treat re-entry from SLEEPING/BLOCKED as no-op-sleep recovery (was masked by BKL-serialised timer ISR); (b) boot task registered as the BSP's `pcpu->idle_task` so `pick_next_task`'s fallback always has a target (symmetric to `scheduler_setup_ap_idle` on APs). **T4 (syscall_entry)** attempted three times this session, all failed under SMP-4 (GP fault / invalid opcode on stack-lifetime races). F7 (departing-task tracking + waitpid reap poll) was a partial fix that closed the parent-reap race but a second stack-lifetime race remains in fork/exec paths. T4/T5 + the lockdep cross-CPU BKL bequeath model fix are coupled and belong in a focused, review-driven PR; see [`docs/m28-t4-blocker.md`](m28-t4-blocker.md) for the failure shapes, F7 details, and a concrete sequence for the next attempt. Smoke 250/0/0 production through T1+T2+T3+T8.
...
- [ ] `partial` Benchmark and optimize context-switch latency under SMP workloads. With T8 (preemption from ISR) landed, the preemptive cost model is now meaningful. A test-mode rdtsc micro-benchmark was prototyped (SYS_GETPID round-trip ≈ 257 cycles, SYS_YIELD ≈ 23k cycles under single CPU) but reverted from the in-tree smoke driver: 1000-iter loops under preemptive `-smp 4` exposed a fault on the high-throughput syscall path (invalid opcode at a heap address — a race the BKL was hiding that doesn't show up in M27/M14 smoke). The benchmark + the race investigation belong together in a focused PR after T4 (BKL out of syscall_entry) actually lands. Full optimization is a follow-up.
