...
- [x] `partial` Audit and remove the Big Kernel Lock (BKL) for userspace execution on APs. Foundation done: lock-order DAG in [`docs/m28-locking.md`](m28-locking.md), debug-only lockdep tracker (`-DKERNEL_LOCKDEP=1`, `kernel/sched/lockdep.c`) instrumenting BKL / `vfs_tree_lock` / `g_tasks_lock` / `heap_lock` / `pmm_lock` / `vmm_lock` / per-inode rw_lock / `bcache_lock` / `fd_lock` / runqueue, TLB shootdown IPI infrastructure (vector 0x41, gated off), reschedule IPI (vector 0x42) wired into `wake_sleepers` + `scheduler_wake_all` + `scheduler_exit_current` (F6). Atomic-CAS state transitions: DEAD→REAPING (F1), SLEEPING/BLOCKED→READY (F4), READY→RUNNING in `pick_next_task` (F5), BLOCKED→READY in exit_current's parent wake (F6). `bkl_unlock` is a no-op for non-owner CPUs (commit 9d0784f). **Done so far: T1 (BKL out of AP idle outer loop)** + **T2 (BKL out of BSP idle)** — both retain `bkl_unlock()` before sti;hlt to release any BKL bequeathed by a syscall-return path. Remaining: T3 (timer ISR — attempted, broke under SMP-4 with a `scheduler_sleep_ticks without running task` panic, needs diagnostic), T4 (syscall_entry), T5 (scheduler_yield core). Pre-existing AP LAPIC-software-enable bug also fixed along the way. Smoke 250/0/0 lockdep off and on through T2.
