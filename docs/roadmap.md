...
- [x] `partial` Audit and remove the Big Kernel Lock (BKL) for userspace execution on APs. Foundation done: lock-order DAG in [`docs/m28-locking.md`](m28-locking.md), lockdep tracker (`-DKERNEL_LOCKDEP=1`) with Variant-A bequeath relaxation (`lockdep_*_global` for BKL + per-inode rw_lock — no false positives on cross-CPU release), TLB shootdown IPI (0x41, gated), reschedule IPI (0x42) wired into `wake_sleepers` / `scheduler_wake_all` / `scheduler_exit_current` (F6). Atomic-CAS state transitions: DEAD→REAPING (F1), SLEEPING/BLOCKED→READY (F4), READY→RUNNING in `pick_next_task` (F5), BLOCKED→READY in exit_current's parent wake (F6). `bkl_unlock` is a no-op for non-owner CPUs. **Done: T1 (AP idle outer loop) + T2 (BSP idle) + T3/T8 BKL-invariant cleanup**: idle loops and timer preemption now enter `scheduler_yield()` with the BKL held and drop it before parking/returning as appropriate. **Primary T4 stack-lifetime race FIXED in tree as a strict improvement**: per-task `stack_released` lease — `arch_context_switch` takes 3rd arg `&old->stack_released` and stores `1` after RSP swap; `scheduler_exit_current` + signal SIGKILL/SIGDFL terminators + `ap_worker_trampoline` clear `0` before `state = TASK_DEAD`; `scheduler_waitpid` polls `==1` before `kfree(stack)`. This closes the M14 `iretq` GP fault deterministically. **Additional strict improvements:** syscall entry no longer uses a global pre-BKL scratch slot for incoming `RAX`; signal death/stop publication now wakes waiters consistently; kernel stacks are 64 KiB for the nested SMP/T8 syscall paths. **T4 (`bkl_lock` removal from syscall_entry.S) still does not land**: after these fixes, the BKL-in baseline passed full smoke `271/0`, but a control T4 run hung in single-CPU M25 after `/tmp/hello` printed and before `M25-SMOKE: ok run-hello`. T4 stabilization needs targeted debugging of that native-toolchain wait/exit shape — out of scope for this branch. See [`docs/m28-t4-blocker.md`](m28-t4-blocker.md) for the full story.
