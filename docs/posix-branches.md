...
- `partial` `open`, `read`, `write`, `close`, `lseek`, `stat`, `lstat`,
  ...
  block-cache flush paths have initial coverage, but full rename semantics
  and durable metadata still need work.
- `partial` Path resolution handles cwd, dot, dot-dot, duplicate slashes, and
  ...
  broader failure tests and cross-mount edge cases are not closed.
...
- `stub` Btrfs is metadata/probe-only and should not be treated as a POSIX
  filesystem yet.
- `partial` Ext3 read/write driver exists with JBD journaling (inode/block
  alloc, dir entries, journal mount and recover); unlink, rename, and
  fsck-friendly metadata are not implemented.
- `partial` Ext4 read/write driver exists with extent tree, 64-bit BGD,
  flex_bg, and JBD journaling; unlink, rename, htree directory indexing, and
  fsck-friendly metadata are not implemented.

### Current Estimate

`VFS/path/files` is roughly 75-82% of the way to a practical POSIX branch
close. The ext1/ext3/ext4 read/write drivers, iterative symlink resolver,
mount-crossing, 18+ wired file I/O syscalls, and errno normalization move
it forward significantly, but the branch is not closed: cross-mount
semantics, permission enforcement consistency, persistent filesystem
