...
4. **In-guest build driver + full build — ATTEMPTED, reached 39/76.** Driver:
...
5. **TWO blockers remain before the build completes:**
   - cc1 executes `ud2` (rip `0x204189d` is a ud2 trap pad in cc1) compiling
     ~file 39 (`kernel/fs/pipe.c`/`vfs.c`) **under KVM** — an abort/ICE or
     corrupted control flow. Under TCG cc1 compiled simpler TUs fine, so this is
     likely KVM-specific (real-CPU FPU/SSE/state exposing a b1nix bug) or an ICE
     on those files. NEXT: compile pipe.c/vfs.c alone under TCG to disambiguate.
   - **Kernel doesn't terminate a process on a fatal default-action signal**
     (SIGILL/SIGFPE): it prints "sending signal N" but returns to the faulting
     instruction → infinite `#UD` loop (32 000+ log lines). Fix the exception
     handler / `arch_check_and_deliver_signals` to apply default-terminate.
6. Add an `M26-SMOKE` test module + boot-log markers for the self-host path.
