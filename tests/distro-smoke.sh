...
KNOWN_DEGRADED="$ROOT_DIR/tools/configs/known-degraded.txt"
...
# -machine pc, not q35: q35 always carries an ICH9 AHCI controller with an
# empty ATAPI port, and probing it is a known hang in this kernel (the soak
# notes call it "AHCI ATAPI hang"). The lane must fail on what it tests, not on
# a bug it did not come to look at; the hang has its own entry to fix.
qemu_boot() { # image serial-log extra-args...
	_img="$1"; _log="$2"; shift 2
	timeout "$BOOT_TIMEOUT" qemu-system-x86_64 \
		-machine pc -m 2048 -smp 2 \
		-drive file="$_img",format=raw,if=virtio \
		-virtfs local,path="$REPO",mount_tag=b1nixrepo,security_model=none,readonly=on \
		-display none -serial file:"$_log" -no-reboot "$@" >>"$LOG" 2>&1 || true
}
...
if [ "$SKIP_BUILD" = "1" ]; then
	...
	sh "$ROOT_DIR/tools/packages/build-deb.sh" >>"$LOG" 2>&1 || stage_die build-packages
	sh "$ROOT_DIR/tools/packages/publish-repo.sh" >>"$LOG" 2>&1 || stage_die publish-repo
	CMDLINE_EXTRA="console=ttyS0 b1nix.smoke" sh "$ROOT_DIR/tools/images/mk-b1nix-image.sh" \
...
if [ "$SKIP_BUILD" != "1" ]; then
	...
	PROFILE=broken CMDLINE_EXTRA="console=ttyS0 b1nix.smoke" \
		sh "$ROOT_DIR/tools/images/mk-b1nix-image.sh" >>"$LOG" 2>&1 || stage_die build-broken-image
fi
