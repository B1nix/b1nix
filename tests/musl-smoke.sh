...
if [ "$RUN_ONLY" = "0" ]; then
  ...
  tools/toolchain/bin/b1nix-musl-cc -dynamic tests/programs/bin/helpers/m92_musl_dyn_test.c -o "$SMOKE_DYN_BIN" 2>&1
  ...
  tools/toolchain/bin/b1nix-musl-cc -ldso tests/programs/bin/helpers/m92_musl_ldso_test.c -o "$SMOKE_LDSO_BIN" 2>&1
