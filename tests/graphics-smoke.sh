...
grep -q "M50-DRM: ok card0" "$LOG"
grep -q "M50-DRM: ok mode" "$LOG"
grep -q "M50-DRM: ok multi-buffer" "$LOG"
grep -q "M50-DRM: ok setcrtc" "$LOG"
grep -q "M50-DRM: ok flip-event" "$LOG"
grep -q "M50-DRM: ok rmfb" "$LOG"
grep -q "M50-DRM: ok cleanup" "$LOG"
grep -q "M51-GFX: ok libm" "$LOG"
grep -q "M51-GFX: ok pixman" "$LOG"
grep -q "M51-GFX: ok freetype" "$LOG"
grep -q "M51-GFX: ok cairo" "$LOG"
grep -q "M51-GFX: ok cairo-wayland" "$LOG"
grep -q "M51-GFX: ok xkbcommon" "$LOG"
grep -q "M51-GFX: ok harfbuzz" "$LOG"
