...
#
# Ships the self-host module (tests/selfhost/build-selfhost-module.sh) as the ram0
...
if [ "${SKIP_PACK:-0}" != "1" ] || [ ! -f "$IMG" ]; then
	...
	sh "$ROOT_DIR/tests/selfhost/build-selfhost-module.sh"
fi
...
"$ROOT_DIR/tools/image/mkiso.sh" "$@" >/dev/null
