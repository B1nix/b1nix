...
#
#   sh tests/selfhost/build-selfhost-module.sh      # after `make` (x86_64)
#
...
TC="$OUT/toolchain"
SELFBUILD="$ROOT_DIR/build/$ARCH/programs/bin/selfhost_build"
...
B1NIX_ARCH=$ARCH ALPINE_LAYOUT=native \
	...
	sh "$ROOT_DIR/tools/image/alpine/alpine-fetch.sh" "$TC" clang17 clang17-headers lld >/dev/null
